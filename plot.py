...
#
# Altered by Bhavesh Mehta to output gcode with the '.gcode' 
# default variables are in metric values
DATE = "11/7/17"
...
(End of sheet header)\n""")
   ...
   for layer in range(len(cad.zwrite)-1,-1,-1):
      ...
      for segment in range(len(sorted_segments)):
         ...
         file.write("G1 X%0.4f "%x+"Y%0.4f "%y+"Z%0.4f"%zup+" F400.00\n") # rapid motion
         ...
         for vertex in range(1,len(sorted_segments[segment])):
            ...
            file.write("G1 X%0.4f "%x+"Y%0.4f"%y+" F400.00\n")
            dxy += sqrt((xold-x)**2+(yold-y)**2)
            xold = x
            yold = y
...
canvas_logo = Canvas(control_frame, width=26, height=26, background="black")
canvas_logo.pack(side='left')
control_text = " CAD_V1.1 (%s) "%DATE
